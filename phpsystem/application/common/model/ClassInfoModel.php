...
class ClassInfoModel extends Model {
    ...
    /*按照查询条件分页查询班级信息*/
    public function queryClassInfo($classNo, $className, $collegeName, $specialName, $mainTeacher, $bornDate, $currentPage) {
        $startIndex = ($currentPage-1) * $this->rows;
        $where = null;
        if($classNo != "") $where['classNo'] = array('like','%'.$classNo.'%');
        if($className != "") $where['className'] = array('like','%'.$className.'%');
        if($collegeName != "") $where['collegeName'] = array('like','%'.$collegeName.'%');
        if($specialName != "") $where['specialName'] = array('like','%'.$specialName.'%');
        if($mainTeacher != "") $where['mainTeacher'] = array('like','%'.$mainTeacher.'%');
    ...
    /*按照查询条件查询所有班级记录*/
  public function queryOutputClassInfo( $classNo,  $className,  $collegeName,  $specialName,  $mainTeacher,  $bornDate) {
        $where = null;
        if($classNo != "") $where['classNo'] = array('like','%'.$classNo.'%');
        if($className != "") $where['className'] = array('like','%'.$className.'%');
        if($collegeName != "") $where['collegeName'] = array('like','%'.$collegeName.'%');
